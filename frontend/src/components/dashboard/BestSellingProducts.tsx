        ...
        <table className="w-full text-sm text-left">
          <thead>
            <tr className="bg-[#EAF8E7] rounded-lg">
              <th className="px-4 py-3 text-[13px] font-medium text-[#6A717F] rounded-l-lg">
                Product
              </th>
              ...
              <th className="px-4 py-3 text-[13px] font-medium text-[#6A717F] rounded-r-lg">
                Price
              </th>
            </tr>
          </thead>
          <tbody>
            {isLoading && (
              <tr>
                <td colSpan={4} className="px-4 py-4 text-sm text-[#6A717F]">
                  Loading…
                </td>
              </tr>
            )}
            {products.map((p) => (
              <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    {p.imageUrl ? (
                      ...
                      <div className="w-[46px] h-[46px] rounded-xl bg-gray-100 flex items-center justify-center text-xs text-gray-400 shrink-0">
                        {p.name[0]}
                      </div>
                    )}
                ...
                <td className="px-4 py-3">
                  <span
                    className={`inline-flex items-center gap-1.5 text-sm font-medium ${p.stock > 0 ? "text-[#21C45D]" : "text-[#EF4343]"}`}
                  >
                    <span
                      className={`w-2 h-2 rounded-full ${p.stock > 0 ? "bg-[#21C45D]" : "bg-[#EF4343]"}`}
                    />
                    {p.stock > 0 ? "Stock" : "Stock out"}
                  </span>
                </td>
                <td className="px-4 py-3 font-bold text-[#023337]">
